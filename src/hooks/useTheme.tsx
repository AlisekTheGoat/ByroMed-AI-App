...
export function ThemeProvider({ children }: ThemeProviderProps) {
  ...
  useEffect(() => {
    ...
    const savedFontFamily =
      (localStorage.getItem("fontFamily") as
        | (
            | "Montserrat"
            | "Tinos"
            | "Sensation"
    ...
    setTheme(savedTheme);
    setFontSize(
      Number.isFinite(savedFontSize) && savedFontSize > 10 && savedFontSize < 24
        ? savedFontSize
        : 16
    );
  ...
  useEffect(() => {
    ...
    const stacks: Record<typeof fontFamily, string> = {
      ...
      "Public Sans":
        '"Public Sans", "Inter", system-ui, -apple-system, "Segoe UI", Roboto, Arial, sans-serif',
      // Sensation is not loaded from Google; use fallback to Inter/system if missing
      Sensation:
        '"Sensation", "Inter", system-ui, -apple-system, "Segoe UI", Roboto, Arial, sans-serif',
    } as const;
